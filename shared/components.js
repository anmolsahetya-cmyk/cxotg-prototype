...
function renderIcon(name, size = 20) {
  const icons = {
    ...
    eye_off: `<svg width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17.94 17.94A10.94 10.94 0 0 1 12 20c-7 0-11-8-11-8a19.68 19.68 0 0 1 5.06-5.94M9.9 4.24A10.4 10.4 0 0 1 12 4c7 0 11 8 11 8a19.8 19.8 0 0 1-3.22 4.4M14.12 14.12a3 3 0 1 1-4.24-4.24"/><line x1="1" y1="1" x2="23" y2="23"/></svg>`,
...
function getManagerByEmail(email) {
  return DATA.managers.find(m => m.email === email) || null;
}

function getManagerByName(name) {
  return DATA.managers.find(m => m.name === name) || null;
}
